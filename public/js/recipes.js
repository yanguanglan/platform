...
angular
	...
	.controller('TopicController', ['$scope', 'filterFilter', 'topic', 'topics', 'Topic', function($scope, filterFilter, topic, topics, Topic) {
		var topicCtl = this;
		...
		topicCtl.toggleVersion = function() {
			...
			Topic.get(topicCtl.topic.uuid, topicCtl.sortByType, topicCtl.version).then(function(data) {
				topicCtl.topic = data;
			});
		};
	...
	.factory('Lesson', ['$http', '$location', function($http, $location) {
		return {
			get: function(uuid) {
				return $http
	...
	.factory('Topic', ['$http', '$location', function($http, $location) {
		var service = {
			all: all,
			get: get
		};
	...
	.factory('Post', ['$http', '$location', function($http, $location) {
		var service = {
			all: all,
			get: get
		};
